---
name: game-dev
description: "Use when building, rebuilding, or substantially extending a Babylon.js browser game through a staged visual-target, architecture, asset, implementation, and verification workflow in a Manus WebDev environment."
---
...
- `references/godogen-stages/godogen-skill-overview.md` — the entry point for the unmodified godogen stage instructions. Read each sibling stage document only when you reach that stage. Treat the stage set as the source of truth for *how a good game is built*; apply the Manus adaptations for *where it runs and how art/deploy happen*.
