...
> **Frozen baseline notice:** This document summarizes the July 2026 planning
> baseline. It predates the curated Phase-1 prototype now tracked under
> `../engine-api/`; current repository and portfolio status take precedence.
...
This summary covers the frozen documentation and planning baseline only. At
that baseline, EI-RAM Analysis Studio was not yet an implemented standalone
application.
...
The 0.1 baseline established the documentation foundation needed before the
later Phase-1 engine import.
...
| Implementation Status | Not implemented at the frozen 0.1 baseline |
| Verification Status | Document/file-presence verification at the frozen baseline |
...
|---|---|---|---|
| LIM-001 | The 0.1 baseline had no standalone EI-RAM Studio app | Could not run the planned workflow at that baseline | Partially superseded by the Phase-1 prototype |
| LIM-002 | The 0.1 baseline referenced but did not integrate the EI-RAM engine | No analysis endpoint existed at that baseline | Superseded by the curated `engine-api/` import |
