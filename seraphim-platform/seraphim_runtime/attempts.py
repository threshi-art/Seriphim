"""G1-10 replay-resistant Runtime attempt creation from accepted task claims."""
...
class AttemptAccessError(LookupError):
    """Intentionally non-disclosing for invalid, stale, or replayed claims."""
...
class AttemptRepository:
    ...
    def create_from_claim(
        ...
        if len(token) != 64 or any(character not in "0123456789abcdef" for character in token):
            raise AttemptAccessError("Accepted claim not found")
        ...
        try:
            ...
            claim = self._connection.execute(
                """
                SELECT task.mission_id, task.claim_expires_at, approval.approval_request_id
            ...
            if claim is None:
                raise AttemptAccessError("Accepted claim not found")
            mission_id, claim_expires_at, approval_request_id = (str(value) for value in claim)
            self._connection.execute(
                ...
                """,
                (attempt_id, task, approval_request_id, worker, token, created_at, claim_expires_at, metadata_json, metadata_digest),
            )
            if failure_injector is not None:
                failure_injector("after_attempt_insert")
            self._append_audit(mission_id, task, attempt_id, approval_request_id, worker, metadata_digest)
            if failure_injector is not None:
                failure_injector("after_audit_insert")
            self._connection.commit()
        except sqlite3.IntegrityError as error:
            self._connection.rollback()
            raise AttemptAccessError("Accepted claim not found") from error
        except Exception:
            self._connection.rollback()
            raise
        return Attempt(attempt_id, task, approval_request_id, worker, token, claim_expires_at, metadata_digest, "created", created_at)

    def _append_audit(self, mission_id: str, task_id: str, attempt_id: str, approval_request_id: str, worker_id: str, metadata_digest: str) -> None:
        ...
        event_hash = hashlib.sha256(f"{sequence}|{previous_hash or ''}|{mission_id}|{task_id}|{worker_id}|attempt.created|success|{metadata_digest}".encode("utf-8")).hexdigest()
        self._connection.execute(
            "INSERT INTO runtime_audit_events(event_sequence, event_id, mission_id, task_id, attempt_id, actor_id, event_type, outcome, payload_digest, previous_event_hash, event_hash, created_at) VALUES (?, ?, ?, ?, ?, ?, 'attempt.created', 'success', ?, ?, ?, ?)",
            (sequence, uuid.uuid4().hex, mission_id, task_id, attempt_id, worker_id, metadata_digest, previous_hash, event_hash, _timestamp()),
        )
