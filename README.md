...
The sanitized public capability projection is regenerated by atomic
replacement. On Windows, the supported security-metadata guarantee preserves
the existing destination DACL and whether that DACL is protected from
inheritance within an operator-controlled local repository directory chain;
failure to copy either stops publication before commit. The destination file
identity is rechecked immediately before commit. A same-Windows-user mutation
after that recheck is within the trusted local boundary and is not claimed as
prevented. File owner, primary group, SACL/audit data, and mandatory integrity
labels are inherited or OS-managed for the replacement file and are outside
this public catalog's threat contract. Generation intentionally does not require
the elevated ownership or security privileges those fields can need, including
in ordinary OneDrive-backed checkouts.
